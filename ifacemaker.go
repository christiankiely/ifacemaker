...
import (
	...
	"github.com/christiankiely/ifacemaker/maker"
)
...
type cmdlineArgs struct {
	...
	copyDocs bool
	SortAlphabetically string `short:"S" long:"sort" description:"Sort methods alphabetically" option:"true" option:"false" default:"false"`
	sortAlphabetically bool
	IncludeTests string `short:"t" long:"tests" description:"Include test files" option:"true" option:"false" default:"false"`
	includeTests bool
...
func main() {
	...
	args.sortAlphabetically = args.SortAlphabetically == "true"
	args.includeTests = args.IncludeTests == "true"
	...
	for _, filePattern := range args.Files {
		...
		for _, match := range matches {
			if !args.includeTests {
				withoutExt := strings.TrimSuffix(match, path.Ext(match))
				if strings.HasSuffix(withoutExt, "_test") {
					continue
				}
			}
	...
	result, err := maker.Make(files, args.StructType, args.Comment, args.PkgName, args.IfaceName, args.IfaceComment, args.copyDocs, args.CopyTypeDoc,args.sortAlphabetically)
	if err != nil {
		log.Fatal(err.Error())
	}
